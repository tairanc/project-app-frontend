...
import flyToCart from "../itemNew/detail.jsx";
import {RNDomain} from 'src/config/index'

import './activity.scss';

const pageApi = {
  discount: { url:"/originapi/promotion/fullDiscount/item", method:"get"},
	rule: { url:"/originapi/promotion/rule" , method:"get" },
	shop: { url: "/originapi/promotion/fullDiscount/cart", method:"get" },
	addCart: { url:"/originapi/cart-add.html", method:"post" },
  channelItem:{ url: "/originapi/itemNew/promotion", method: "get"},
...
class DiscountActivity extends Component{
	constructor( props, context ){
		super( props );
		document.title = "满折专场";
		if( context.isApp ) location.href = "jsbridge://set_title?title=满折专场";
	...
	dropDown =( me )=>{
		...
		axios.request({ ...pageApi.discount,
			params:{
	...
	render(){
		...
		return <div data-page="promotion-activity" style={{ minHeight:$(window).height(), background:"#f4f4f4" }}>
			...
			<DropDownList scrollArea={ window } dropDown={ this.dropDown } isLogin={ this.context.isLogin } data={ this.props.list.data } itemData={this.props.itemData}  getChannelItem={ this.props.getChannelItem  } isApp={ this.context.isApp } />
...
class PageHead extends Component{
	parseRule(){
		...
		return rules.map(( list, i )=>{
			return list.full +"件" + list.discount/10 + "折";
		}).join("，");
	}
	render(){
		return <div className="page-head">
			<div className="main-text">以下商品 { this.parseRule() }</div>
...
function discountActivityState ( state ) {
	return {
    ...state.minusActivity,
    // ...state.global
	}
...
function discountActivityDispatch ( dispatch, props ) {
...
export default connect( discountActivityState, discountActivityDispatch )( DiscountActivity );
