...
const pageApi = {
    exchangeBuy: { url:"/newapi/promotion/exchangeBuy/item", method:"get"},//频道商品
    rule: { url:"/newapi/promotion/rule" , method:"get" },
    shop: { url: "/newapi/promotion/exchangeBuy/cart", method:"get" },//频道购物车
    addCart: { url:"/originapi/cart-add.html", method:"post" },//商品加入购物车
...
class ExchangeBuyActivity extends Component{
    constructor( props, context ){
        super( props );
        document.title = "加价换购";
        if( context.isApp ) location.href = "jsbridge://set_title?title=加价换购";
    ...
    dropDown =( me )=>{
        ...
        axios.request({ ...pageApi.exchangeBuy,
            params:{
    ...
    render(){
        ...
        return <div data-page="promotion-activity" style={{ minHeight:$(window).height(), background:"#f4f4f4" }}>
            ...
            <DropDownList scrollArea={ window } dropDown={ this.dropDown } isLogin={ this.context.isLogin } data={ this.props.list.data } 
...
class PageHead extends Component{
    parseRule(){
        let rules = JSON.parse( this.props.rules.rules).rule;
        return rules.exchange_full +"元可换购热销商品";
    }
    render(){
...
class PageMain extends Component{
    ...
    getHtml(){
        return this.props.data.map( ( list, i )=>{
            return <OneItem key={ i } data={ list } getChannelItem ={ this.props.getChannelItem  } id={list.item_id}  
                            itemData={this.props.itemData} isApp={ this.props.isApp } isLogin={ this.props.isLogin } />
        });
    }
    render(){
...
function exchangeBuyActivityState ( state ) {
    return {
      ...state.minusActivity,
      // ...state.global
    }
...
function exchangeBuyActivityDispatch ( dispatch, props ) {
...
export default connect( exchangeBuyActivityState, exchangeBuyActivityDispatch )( ExchangeBuyActivity );
